...
-- Vincula mentores escalados a cada classe (professor e/ou host/mentor).
-- weekday permite o mesmo mentor ter papéis em dias diferentes da semana.
CREATE TABLE IF NOT EXISTS class_mentors (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  class_id UUID NOT NULL REFERENCES classes(id) ON DELETE CASCADE,
  mentor_id UUID NOT NULL REFERENCES mentors(id) ON DELETE CASCADE,
  role TEXT NOT NULL DEFAULT 'Professor' CHECK (role IN ('Professor', 'Host', 'Mentor')),
  weekday SMALLINT,
  created_at TIMESTAMPTZ DEFAULT now(),
