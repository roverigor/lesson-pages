-- ═══════════════════════════════════════════════════════════════════════════
-- nps_results_by_survey — Group responses by form/session for dashboard split
--
-- Output enriched para uso por diretoria + professores:
--   • cohort_id + cohort_name              (qual turma)
--   • Aula NN ordinal pra classes regulares (via zoom_meetings ordenadas)
--   • Label intuitivo: "<cohort> · <descritor>"
--   • Split DM vs Grupo (dm_total / group_total / dm_nps / group_nps)
--
-- Regras label:
--   • manual_survey  → "<cohort> · <survey.name>"
--   • auto_class regular → "<cohort> · Aula <NN> — DD/MM (<class.name>)"
--   • auto_class PS      → "<cohort> · <class.name> — DD/MM"
-- ═══════════════════════════════════════════════════════════════════════════
...
RETURNS TABLE (
  ...
  group_key     TEXT,        -- survey_id (manual)  OR  'session:<class_id>:<cohort_id>:<YYYY-MM-DD>' (auto)
  label         TEXT,        -- intuitive friendly name
  survey_id     UUID,
  class_id      UUID,
  class_name    TEXT,
  class_kind    TEXT,        -- 'regular' | 'ps' | NULL
  cohort_id     UUID,
  cohort_name   TEXT,
  session_date  DATE,
  session_index INT,         -- "Aula N" ordinal for regular classes; NULL for PS/manual
  total         INT,
  dm_total      INT,
  group_total   INT,
  nps           NUMERIC,
...
BEGIN
  ...
  -- Compute session ordinal (Aula NN) using zoom_meetings for non-PS classes
  auto_with_index AS (
    SELECT
      a.*,
      cl.name AS class_name,
  ...
  SELECT
    ...
    COALESCE(a.cohort_name, 'Sem turma') || ' · ' ||
      CASE
        WHEN a.class_kind = 'ps' THEN COALESCE(a.class_name, 'PS') || ' — ' || to_char(a.session_date, 'DD/MM')
        WHEN a.session_index IS NOT NULL THEN 'Aula ' || lpad(a.session_index::text, 2, '0') || ' — ' || to_char(a.session_date, 'DD/MM') || ' (' || COALESCE(a.class_name, 'Aula') || ')'
...
COMMENT ON FUNCTION public.nps_results_by_survey(JSONB) IS
  'NPS Results — breakdown por formulário/sessão com cohort + Aula NN + DM/Grupo split. Manual surveys agrupam por survey_id; auto pós-aula agrupa por (class_id, cohort_id, day BRT). Aula NN ordinal computado por zoom_meetings ordenadas (regular classes apenas; PS = data direta).';
