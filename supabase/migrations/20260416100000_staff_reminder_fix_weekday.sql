-- ═══════════════════════════════════════
-- Fix: use class_mentors.weekday instead of classes.weekday
-- Classes like PS Fundamentals run on Tue+Fri but classes.weekday=2 (Tue only)
-- Staff assignments in class_mentors have the correct weekday per assignment
-- ═══════════════════════════════════════
...
BEGIN
  ...
  FOR rec IN
    ...
    JOIN class_mentors cm ON c.id = cm.class_id
      ...
      AND cm.weekday = today_dow              -- ← match on assignment weekday
  ...
  LOOP
    ...
    -- Build companion info (Professor sees Host, Host sees Professor)
    companion_line := '';
