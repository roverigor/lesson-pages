-- ═══════════════════════════════════════════════════════════════════════════
-- dispatch_history_unified — add 5th source: ps_rsvp_links (PS RSVP DMs)
--
-- Bug: /admin/envios/ não mostrava disparos PS RSVP (dispatch-ps-rsvp cron).
-- ═══════════════════════════════════════════════════════════════════════════
...
UNION ALL
 SELECT 'ps_rsvp_link'::text AS source,
    p.id AS dispatch_id,
        CASE
            WHEN (p.meta_message_id IS NOT NULL) THEN 'meta_dm'::text
            ELSE 'evolution_dm'::text
        END AS channel,
    COALESCE(p.sent_at, p.created_at) AS sent_at,
    NULL::timestamp with time zone AS delivered_at,
    NULL::timestamp with time zone AS read_at,
        CASE
            WHEN (p.responded_at IS NOT NULL) THEN 'responded'::text
            ELSE COALESCE(p.send_status, 'pending'::text)
        END AS status,
    p.error_detail,
    p.student_id,
    NULL::uuid AS mentor_id,
    ...
    'ps_rsvp'::text AS dispatch_type,
    NULL::text AS template_name,
    'utility'::text AS template_category,
    NULL::text AS rendered_message,
    COALESCE(p.meta_message_id, p.evolution_message_id) AS provider_message_id,
    jsonb_build_object('token', (p.token)::text, 'session_date', p.session_date, 'expires_at', p.expires_at, 'responded_at', p.responded_at, 'evolution_message_id', p.evolution_message_id, 'meta_message_id', p.meta_message_id) AS metadata,
    0 AS open_count,
    NULL::timestamp with time zone AS last_opened_at,
        CASE
            WHEN (p.responded_at IS NOT NULL) THEN 1
            ELSE 0
        END AS response_count,
    p.created_at
   FROM ps_rsvp_links p;
