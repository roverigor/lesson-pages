-- ═══════════════════════════════════════════════════════════════════════════
-- NPS.P.3 — Results aggregation RPCs for admin/nps-results/ dashboard
-- Multi-stakeholder consumers: profs, CS, equipe educacional.
-- ═══════════════════════════════════════════════════════════════════════════

-- ─── Summary KPIs over filter window ───
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN COUNT(*) > 0
      THEN ROUND(100.0 * (COUNT(*) FILTER (WHERE nps_score >= 9) - COUNT(*) FILTER (WHERE nps_score <= 6)) / COUNT(*), 1)
      ELSE NULL END,
    ROUND(AVG(nps_score)::numeric, 2)
  FROM base
  GROUP BY wk
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*)::int,
    CASE WHEN COUNT(*) > 0
      THEN ROUND(100.0 * (COUNT(*) FILTER (WHERE r.nps_score >= 9) - COUNT(*) FILTER (WHERE r.nps_score <= 6)) / COUNT(*), 1)
      ELSE NULL END,
    ROUND(AVG(r.nps_score)::numeric, 2),
    COUNT(*) FILTER (WHERE r.nps_score <= 6)::int,
    COUNT(*) FILTER (WHERE r.nps_score >= 9)::int
  FROM public.class_nps_responses r
...
-- ─── Comments list (detractors emphasized) ───
...
BEGIN
  ...
  SELECT
    ...
    r.nps_score,
    CASE
      WHEN r.nps_score >= 9 THEN 'promoter'
      WHEN r.nps_score >= 7 THEN 'passive'
      ELSE 'detractor'
    END,
    r.comment,
    r.mode,
    ...
    co.name,
    cl.name,
    CASE WHEN r.mode = 'dm' THEN s.name ELSE NULL END,
    CASE WHEN r.mode = 'dm' THEN s.phone ELSE NULL END
  FROM public.class_nps_responses r
  ...
  WHERE
    ...
    AND (p_filters->>'bucket' IS NULL OR
         (p_filters->>'bucket' = 'detractor' AND r.nps_score <= 6) OR
         (p_filters->>'bucket' = 'passive' AND r.nps_score BETWEEN 7 AND 8) OR
         (p_filters->>'bucket' = 'promoter' AND r.nps_score >= 9))
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', id, 'name', name) ORDER BY name), '[]'::jsonb)
    INTO v_cohorts
  FROM public.cohorts
  WHERE EXISTS (SELECT 1 FROM public.class_nps_responses r WHERE r.cohort_id = cohorts.id);

  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', id, 'name', name) ORDER BY name), '[]'::jsonb)
    INTO v_classes
  FROM public.classes
  WHERE EXISTS (SELECT 1 FROM public.class_nps_responses r WHERE r.class_id = classes.id);
